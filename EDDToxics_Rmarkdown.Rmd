...
kable(chars, format='latex', caption="List of Analyzed Pollutants", booktabs=TRUE,longtable=TRUE) %>%
 kable_styling(latex_option="HOLD_position","reapeat_header")
...
##Estimated Data
Data may be estimated for a variety of reasons including issues with quality control, sampling, hold time, and/or storage that causes the data to be biased high or low. This table does not include "J flag" data, ie results that fall between the method detection limit and quantitation limit, as this data . Estimated data may still be usable for permitting purposes, but must be used with caution. &nbsp;

``` {r echo=FALSE, results='asis'}
est<-params$estimate
est<-subset(est,select=c(act_id,Char_Name,Result,Result_Unit,Result_Comment))
names(est)<-c("Activity ID","Pollutant","Result","Result_Unit","Result_Comment")
if(nrow(est)!=0) {kable(est,format='latex', caption="Estimated Data (not including J flag data)",booktabs=TRUE, row.names=FALSE,
                        longtable=TRUE) %>%
                  kable_styling(latex_options=c("HOLD_position","repeat_header")) %>%
                  column_spec(5,"7cm")%>%
                  column_spec(2,"4cm")} else {cat("**NO KNOWN ISSUES**")}
