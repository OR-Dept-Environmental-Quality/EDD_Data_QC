...
dff<-namefrac(dff)
dff<-subset(dff,select=c('MLocID.x','SampleStartDate.x','Char_Name','Result', 'MRLValue','Result_Unit','diff','RPD'))
...
if(nrow(dff)!=0) {kable(dff, format='latex', caption="Metals with dissolved fraction larger than total recoverable fraction", booktabs=TRUE, row.names=FALSE,longtable=TRUE) %>%
  kable_styling(latex_options=c("HOLD_position","striped","repeat_header"))} else {cat("**NO KNOWN ISSUES**")}
