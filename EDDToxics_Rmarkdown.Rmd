...
actgrp<-info %>% group_by(SampleStartDate, MLocID) %>%
  summarize(type=paste(sort(unique(act_id)),collapse=", "))
...
dff<-namefrac(dff)
dff<-subset(dff,select=c(act_id.x,Char_Name,Result, MRLValue,Result_Unit,diff,RPD))
...
#change names
names(dff)<-c("Activty ID", "Pollutant", "Result","MRL","Unit", "Difference","RPD")
