...
## Quantitation Limits 
A quantitation limit (QL) is the lowest amount of a pollutant that can be reliably measured. Permittees are required to use the most sensitive analytical test method, as specified in 40 CFR 122.44(i)(1)(iv). Established QLs were adopted in 2007 for NPDES permits and updated in 2019 by DEQ WQ Permitting and Program Development. 
...
if (nrow(qlc)!=0) {cat("The following table includes a list of pollutants for which the provided MRLs do not meet the recommended QLs and thus may not be sufficiently sensitive. This table uses the current established QLs that were updated in 2019. The MRL column is the value submitted by the lab as the lowest value the instrument can reliably measure. The MDL column is the value submitted by the lab as the lowest value that the instrument can detect an analyte.")}
