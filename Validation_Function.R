...
as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}
  ...
  ql<-function(x){
    ...
    x$issue<-ifelse(
      x$MRLValue>x$QL & (x$Result_Numeric<x$MRLValue | x$Result_Text=="ND"| (x$Result_Numeric==x$MRLValue & (is.na(x$MDLValue)|x$MRLValue==x$MDLValue)) 
                         |(x$Result_Numeric==x$MRLValue & is.na(x$MDLValue))) & !(x$MDLValue<x$QL & x$Result_Operator=="<"),
      "QL not met",
      NA)
    
    return(subset(x,!(is.na(issue)),
           select=c("Char_Name","CASNumber","act_id","Result_Text","Result_Unit","MDLValue","MRLValue","MRLUnit","QL","QL_Unit","Result_Type","Result_Comment","issue")))
  ...
  dvst<-function(x){
    ...
    final<-unique(subset(cont,select=c("Char_Name.x","Sample_Fraction","Result_Text","MRLValue","MDLValue","Result_Unit","diff","RPD","SampleStartDate.x",
    ...
    names<-c("Char_Name.x","Sample_Fraction","Result_Text","MRLValue","MDLValue","Result_Unit","diff","RPD","SampleStartDate.x",
    "SampleStartTime","OrganizationID","MLocID.x","Project1","Result_status","Result_Comment")
  ...
  mc<-function(x){
    ...
    x$CFR_Method<-ifelse(x$Char_Name %in% c("Tributyl phosphate","Salinity","Demeton","Arsenic, Inorganic",
                                            ...
                                            'Bromoethane','Arsenic ion (3+)','Arsenic ion (5+)',"N-Nitrosomorpholine","N-Nitrosomethylethylamine",
                                            "N-Nitrosopiperidine", "Coumaphos","Dicrotophos", "Ethoprop", "Famphur", "Fenamiphos", "Fensulfothion",
                                            "Leptophos", "Methyl parathion", "Monocrotophos", "O-Ethyl O-(p-nitrophenyl) phenylphosphonothioate",
                                            "O,O,O-Triethyl phosphorothioate", "Prothiofos", "Ronnel", "Sulfotep", "Sulprofos", "Tetrachlorvinphos",
                                            "Aroclor 1262","Aroclor 1268", "Benzo[j]fluoranthene", "Dibenzo[a,i]pyrene", "3-Methylcholanthrene",
                                            "Dibenzo[a,h]pyrene", "Dibenz[a,h]acridine", "	2,2,4,5,6,7,8,8-Octachloro-2,3,3a,4,7,7a-hexahydro-4,7-methano-1H-indene",
                                            "Sum of alpha-, beta-, gamma- and delta-BHC", "Dibenzo[a,e]pyrene", "Dibenz[a,j]acridine", "Perylene", "Styrene",
                                            "O-Dinitrobenzene", "m-Dinitrobenzene", "p-Dinitrobenzene", "2,3,5,6-Tetrachlorophenol", "Azinphos-methyl", "Chlorfenvinphos",
                                            "Dicrotophos"),
  ...
  estm<-function(x){
    ...
    sub<- subset(x,x$Result_Type=="Estimated" & x$Result_status!="Rejected" & 
                  ...
                  select=c(MLocID,act_id,SampleStartDate,SampleStartTime,Char_Name,Char_Speciation,
                           CASNumber,Result_Text,Result_Unit,Method_Code,Result_Comment,Result_Type,Result_status))
  }
  ...
  rej<-function(x){
    ...
    sub<-subset(x,Result_status %in% c("Rejected"),
                select=c(MLocID,act_id,SampleStartDate,SampleStartTime,Char_Name,Char_Speciation,
                         CASNumber,Result_Text,Result_Unit,Method_Code,Result_Comment,Result_status))
  }
  


#x<-AWQMS_Data(startdate = "2021-06-01", enddate = "2021-07-13" , org = "WILL_FALLS_PAPER(NOSTORETID)")
